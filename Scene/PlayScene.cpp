...
const std::vector<int> PlayScene::code = { ALLEGRO_KEY_UP, ALLEGRO_KEY_ENTER };
...
void PlayScene::Hit(int dmg) {
	lives-=dmg;
	UILives->Text = std::string("Hp :") + std::to_string(lives);
	if (lives <= 0) {
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		que.pop();
        if(p.x<MapWidth&&p.x>=0&&p.y-1<MapHeight&&p.y-1>=0)
        {
        ...
        if(p.x-1<MapWidth&&p.x-1>=0&&p.y<MapHeight&&p.y>=0)
        {
